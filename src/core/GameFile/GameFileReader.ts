...
export class GameFileReader extends BymlReader {
  ...
  public getConnections() {
    ...
    for (let i = 0; i < connectionNum; i++) {
      ...
      connections.push(new GameConnection(nodeId, idA, socketA, idB, socketB));
    }
    return connections;
  }
  ...
  private parseNodon(node: BymlHash) {
    ...
    const propsI32 = this.parseNumberArray(propNode, Key.mS32) as GameNodonI32Props;
    const propsU32 = this.parseNumberArray(propNode, Key.mU32) as GameNodonU32Props;
    const propsF32 = this.parseNumberArray(propNode, Key.mF32) as GameNodonF32Props;
    const propsU64 = this.parseNumberArray<bigint>(propNode, Key.mU64) as GameNodonU64Props;
